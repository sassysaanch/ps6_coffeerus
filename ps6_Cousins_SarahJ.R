...
## -----------------------------------------------------------------------------
    ...
    for (i in 1:length(files)) {
      writeLines(str_c('i=', i, '; files[', i, ']=', files[i], sep = '')) 
      unzip(zipfile = file.path(data_dir, str_c(files[i], '.zip')),exdir = file.path(data_dir))
...
for (i in seq_along(files)) {
  ...
  unzip(zipfile = file.path(data_dir, str_c(files[[i]],".zip")), 
        exdir = data_dir) 
  df <- read_csv(file = file.path(data_dir, str_c(files[i], '.csv')))
  dfs[[i]]<- df #code to store each df object created within the loop to the dfs object
  }
